                        ...
                        <tr key={sale._id} className="text-center text-gray-800">
                                    ...
                                    {sale.items.map(item => {
                                        ...
                                        return (
                                            <li key={item.productId}>
                                                {item.productName} R${item.price} - {item.quantity}{product?.description ? `${product.description}` : ""}
                                            </li>
                                        )
                                    })}
                                </ul>
                            ...
                            <td className="p-2 border">
                                <select
                                    ...
                                    ${sale.status === "Pago" ? "bg-green-100 text-green-700 border-green-400" : ""}`}
                                    value={sale.status}
                                    onChange={e =>
                                        handleStatusChange(sale._id, e.target.value as "Em aberto" | "Pago")
                                    }
                                >

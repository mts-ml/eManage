...
export const Receivables: React.FC = () => {
    ...
    const [editFormData, setEditFormData] = useState<UpdateReceivableRequest>({
        status: "Pendente",
        totalPaid: 0,
        remainingAmount: 0,
        firstPaymentDate: "",
    ...
    const sortReceivables = (data: Receivable[], config: { field: keyof Receivable, order: 'asc' | 'desc' }): Receivable[] => {
        return [...data].sort((a, b) => {
            ...
            switch (config.field) {
                ...
                case 'status':
                    ...
                    break
                case 'totalPaid':
                    aValue = Number(a.totalPaid)
                    bValue = Number(b.totalPaid)
                    break
                case 'remainingAmount':
                    aValue = Number(a.remainingAmount)
                    bValue = Number(b.remainingAmount)
                    break
                default:
                    return 0
            }
    ...
    const getSortIcon = (field: keyof Receivable) => {
    ...
    const validateEditForm = (): boolean => {
        const errors: Partial<Record<keyof UpdateReceivableRequest, string>> = {}
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            await axiosPrivate.patch(`/receivables/${editingSale._id}`, updateData)
            ...
            logError("Receivables", error);
            setEditErrors(prev => ({ ...prev, observations: "Erro ao atualizar venda. Tente novamente." }))
        } finally {
            setIsSaving(false)
        }
    }
                ...
                <section className="fixed inset-0 bg-white/95 backdrop-blur-sm flex items-start justify-center z-50 p-4 overflow-y-auto">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-[95vh] overflow-y-auto border-2 border-emerald-200/50">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            {/* Erro geral */}
                            {editErrors.observations && (
                                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                                    <p className="text-red-700 text-sm">{editErrors.observations}</p>
                                </div>
                            )}
